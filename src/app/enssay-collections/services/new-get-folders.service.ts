...
export class NewGetFoldersService {
  ...
  parseJsons(path: string, inference: boolean): Observable<ParsedJsonResult[]> {
    ...
    return this.http.get<ParsedJsonResult[]>(`${NEW_BASE_URL}parse-jsons`, {
      params,
    });
  }

  getImageNewApi(): Observable<string[]> {
  ...
  getImages(
    folderName: string
  ): Observable<{ name: string; base64: string }[]> {
    return this.http.get<{ name: string; base64: string }[]>(
      `${BASE_URL}/images`,
      {
  ...
  download(directoryPath: string): Observable<Blob> {
    ...
    return this.http.get<Blob>(`${NEW_BASE_URL}/download`, {
      params,
